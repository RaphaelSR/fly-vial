...
void main(){
  vLocal = aPos.xz;
  gl_Position = uVP * vec4(aPos.x * uRadius, 0.0, aPos.z * uRadius, 1.0);
}`;

/* the food medium in the bottom of a culture vial: dark, matte, slightly mottled */
...
void main(){
  ...
  if (r > 1.0) discard;
  float n = noise(vLocal * 14.0) * 0.5 + noise(vLocal * 38.0) * 0.3;
  vec3 medium = mix(vec3(0.092, 0.070, 0.048), vec3(0.150, 0.118, 0.078), n);
  medium *= 0.75 + 0.35 * smoothstep(1.0, 0.35, r);       // darker towards the wall
  frag = vec4(medium, 1.0);
}`;

/* the glass: seen from inside, so the far wall is what reads */
const WALLVS = `#version 300 es
precision highp float;
in vec3 aPos;
uniform mat4 uVP; uniform float uRadius; uniform float uHeight;
out vec3 vP; out float vY; out vec3 vN;
void main(){
  vec3 p = vec3(aPos.x * uRadius, aPos.y * uHeight, aPos.z * uRadius);
  vP = p; vY = aPos.y; vN = normalize(vec3(aPos.x, 0.0, aPos.z));
  gl_Position = uVP * vec4(p, 1.0);
}`;
const WALLFS = `#version 300 es
precision highp float;
in vec3 vP; in float vY; in vec3 vN;
uniform vec3 uEye;
out vec4 frag;
void main(){
  vec3 V = normalize(uEye - vP);
  float fres = pow(1.0 - abs(dot(normalize(vN), V)), 2.6);
  float band = smoothstep(0.02, 0.10, abs(fract(vY * 5.0) - 0.5));   // moulding rings
  vec3 glass = vec3(0.34, 0.50, 0.55) * (0.10 + 0.85 * fres) + vec3(0.10, 0.16, 0.18) * band * 0.25;
  float a = 0.05 + 0.55 * fres;
  frag = vec4(glass, a);
}`;
...
function cylinder(seg = 56) {
  const pos = [], idx = [];
  for (let i = 0; i <= seg; i++) {
    const a = (i / seg) * Math.PI * 2, c = Math.cos(a), s2 = Math.sin(a);
    pos.push(c, 0, s2, c, 1, s2);
  }
  for (let i = 0; i < seg; i++) {
    const k = i * 2;
...
const STRIDE = 0.30;
export const VIAL_R = 2.7;   // inner radius of the culture vial, in body lengths
const VIAL_H = 2.4;
...
export class FlyView {
  constructor(canvas) {
    ...
    this.wallprog = link(gl, WALLVS, WALLFS);
    this.sphere = mesh(gl, this.prog, sphere());
    this.wing = mesh(gl, this.wprog, wingMesh());
    this.ground = mesh(gl, this.gprog, disc());
    this.wall = mesh(gl, this.wallprog, cylinder());
    this.u = {
    ...
    this.gu = { VP: gl.getUniformLocation(this.gprog,'uVP'), radius: gl.getUniformLocation(this.gprog,'uRadius') };
    this.lu = { VP: gl.getUniformLocation(this.wallprog,'uVP'), radius: gl.getUniformLocation(this.wallprog,'uRadius'),
                height: gl.getUniformLocation(this.wallprog,'uHeight'), eye: gl.getUniformLocation(this.wallprog,'uEye') };

    this.yaw = 0.75; this.pitch = -0.22; this.dist = 6.2; this.userMoved = false;
    this.camAt = [0, 0, 0];
    this.s = {
  ...
  _bind(){
    ...
    c.addEventListener('wheel',e=>{e.preventDefault();
      this.dist=Math.max(2.2,Math.min(13,this.dist*Math.exp(e.deltaY*0.0012)));},{passive:false});
  }
  ...
  update(d, dt){
    ...
    // the vial wall: slide along it and steer back inward instead of clipping out
    const r = Math.hypot(s.pos[0], s.pos[2]);
    const limit = VIAL_R - 0.42;
    if (r > limit) {
      const k = limit / r;
      s.pos[0] *= k; s.pos[2] *= k;
      const inward = Math.atan2(-s.pos[0], -s.pos[2]);
      let diff = ((inward - s.heading + Math.PI * 3) % (Math.PI * 2)) - Math.PI;
      s.heading += diff * Math.min(1, dt * 2.6);
    }
  ...
  draw(){
    ...
    // the vial is the stage: the camera watches it, drifting only slightly with her
    const want = [s.pos[0] * 0.28, s.pos[1] * 0.5, s.pos[2] * 0.28];
    for (let a = 0; a < 3; a++) this.camAt[a] += (want[a] - this.camAt[a]) * 0.05;
    ...
    // the medium in the bottom of the vial
    gl.useProgram(this.gprog);
    gl.disable(gl.BLEND);
    gl.uniformMatrix4fv(this.gu.VP, false, vp);
    gl.uniform1f(this.gu.radius, VIAL_R);
    ...
    // the glass, drawn last and not writing depth so she stays visible through it
    gl.useProgram(this.wallprog);
    gl.enable(gl.BLEND); gl.blendFunc(gl.SRC_ALPHA, gl.ONE_MINUS_SRC_ALPHA);
    gl.depthMask(false);
    gl.uniformMatrix4fv(this.lu.VP, false, vp);
    gl.uniform1f(this.lu.radius, VIAL_R);
    gl.uniform1f(this.lu.height, VIAL_H);
    gl.uniform3fv(this.lu.eye, eye);
    gl.bindVertexArray(this.wall.vao);
    gl.drawElements(gl.TRIANGLES, this.wall.count, gl.UNSIGNED_SHORT, 0);
    gl.depthMask(true);
